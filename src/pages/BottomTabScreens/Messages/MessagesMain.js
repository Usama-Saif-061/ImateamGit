...
import { View, Alert, Text, FlatList, StyleSheet, TouchableOpacity, ActivityIndicator } from "react-native";
...
import { deleteChat, getAllConversations, getAllMessages } from "./API/messagesApi";
import { useDispatch, useSelector } from "react-redux";
import { setExistingChats, startLoader } from '../../../Reducers/CommonReducers/chatSlice'
...
const MessagesMain = ({ navigation }) => {
    ...
    useEffect(() => {
        console.log("api call for new messages")
        getAllChats(false)
    ...
    const getAllChats = async (bool) => {
        try {
            ...
            if (res.resultCode == 200) {
                ...
                let temp = [...res.data]
                var arr1 = temp.sort((a, b) => moment(b.last_message?.updated_at) - (moment(a.last_message?.updated_at)))
                setChatList(arr1)
            } else {
    ...
    const rightActionSwipe = (item) => {
        return (
            <TouchableOpacity style={styles.deleteView} onPress={() => {
                setShowPopUp(true);
                setDelKey(item.id)
            }} >
                <View>
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            if (res.resultCode == 200) {
                ...
                Alert.alert('Error', res.message?.message ? res.message?.message : 'Error while deleting Conversation')
            }
        } catch (e) {
