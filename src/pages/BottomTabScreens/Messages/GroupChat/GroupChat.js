...
import { getAllMessages, SingleMessageApi, updateUnreadMsgsApi } from '../API/messagesApi'
...
const GroupChat = ({ navigation, route }) => {
    ...
    const getAllMessagesFunc = async (loader = false) => {
        try {
            ...
            if (res.resultCode == 200) {
                ...
                Alert.alert('Error', res.message?.message ? res.message?.message : 'Error while getting Chat Messages')
            }
        } catch (e) {
    ...
    const updateUnreadMsgsFunc = async () => {
        ...
        try {
            ...
            if (res.resultCode == 200) {
                ...
                Alert.alert('Error', res.message?.message ? res.message?.message : 'Error while updating unread messages')
            }
        } catch (e) {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            ...
            <GroupHeader
                title={chatObj.name !== '' ? chatObj.name : chatObj.users[0]?.display_name}
                chatObj={chatObj}
                    ...
                    <View style={{ flex: 1 }}>
                        {
                            ...
                            <FlatList
                                ...
                                renderItem={({ item }) =>
                                    <IndividualSingleMessage obj={item} />
                                }
                            />
                        }
                        {
